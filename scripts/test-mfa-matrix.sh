...
try() { # try <label> <user> <password> <otp> <expect: GRANT|DENY>
  ...
  local C=(-sk -c "$jar" -b "$jar" --resolve "${WL_WEBTOP_FQDN}:443:${WL_APM_VIP}")
...
try "alice: correct pw + her own OTP"      alice.admin "$WL_TEST_USER_PW" "$(oathtool --totp -b "$A_SEC")" GRANT
try "bob:   correct pw + his own OTP"      bob.user    "$WL_TEST_USER_PW" "$(oathtool --totp -b "$B_SEC")" GRANT
echo
echo "=== the cases that must fail ==="
try "alice: correct pw + BOB's OTP"        alice.admin "$WL_TEST_USER_PW" "$(oathtool --totp -b "$B_SEC")" DENY
try "alice: correct pw + wrong OTP"        alice.admin "$WL_TEST_USER_PW" "000000"                          DENY
try "alice: WRONG pw + her own OTP"        alice.admin "not-her-password" "$(oathtool --totp -b "$A_SEC")"  DENY
try "alice: correct pw + NO OTP"           alice.admin "$WL_TEST_USER_PW" ""                                DENY
