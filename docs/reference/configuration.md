...
from it: the DNS zone, the directory entries and every BIG-IP object. Nothing else in the
repo hardcodes a DN, a port or an address.

This page follows `.env.example` section for section, so the file and its reference read in
the same order:

| § | Section | When it applies |
|---|---|---|
| 1 | [This host](#1-this-host) | always |
| 2 | [Directory — both modes](#2-directory--both-modes) | always |
| 3 | [Directory — bundled only](#3-directory--bundled-only) | `MFA_DIRECTORY_MODE=bundled` |
| 4 | [Directory — external only](#4-directory--external-only) | `MFA_DIRECTORY_MODE=external` |
| 5 | [The BIG-IP](#5-the-big-ip) | skipped by `./deploy.sh --stack` |
| 6 | [One-time codes](#6-one-time-codes) | always, all defaulted |
| 7 | [Certificates and rarely-changed overrides](#7-certificates-and-rarely-changed-overrides) | always, all defaulted |

Four rules make the rest of this page readable:
...
- **Commented out in `.env.example`** means the key has a working default, and the file shows
  it. Uncomment only to override. The derivation lives in one place
  ([`scripts/lib/directory.sh`](../../scripts/lib/directory.sh)), so a value set once is
  consistent across the deploy, the BIG-IP build and the validator.
- **Empty is meaningful** for the keys that ship uncommented but blank. `BIGIP_B_MGMT` and
  `BIGIP_B_TMUI` empty select the single-unit deployment; `MFA_DEVICE_GROUP` empty means
  "discover the device group"; an empty `BIGIP_PASS` means "inject it from the environment".
- **`.env` is sourced with `set -a`**, so every key becomes an exported environment
  ...
  the injected value before sourcing `.env` and restore it afterwards. It is also why any
  value containing a space must be quoted, and why an unquoted `<angle-bracket>` placeholder
  fails the whole file as a shell redirect.

## 1. This host
| Variable | Default | Meaning |
|---|---|---|
| `MFA_HOST_IP` | _(required)_ | Address of the Docker host running the directory and DNS, as the BIG-IP reaches it |
...
(the leaf certificates are reissued, the CA is kept) and then `./deploy.sh --bigip`.
Otherwise the old SANs stay in place and fail closed — and a SAN mismatch surfaces as an
...
| Variable | Default | Meaning |
|---|---|---|
| `MFA_DNS_UPSTREAM` | `1.1.1.1` | Where names outside the demo zone are forwarded. No built-in default: leave it set |
| `MFA_DNS_PORT` | `53` | Host port CoreDNS binds on `MFA_HOST_IP`, UDP and TCP. Change it if something already owns `:53` |

A DNS server is part of this stack because the BIG-IP resolves the demo names through a TMOS
`dns-resolver`, which does its own lookups in TMM and does **not** read the BIG-IP's
`/etc/hosts`. A hosts-file entry can cover the browser; it can never cover the BIG-IP.
Rather than make the demo depend on a DNS server you may not be permitted to edit,
bigip-mgt-mfa ships CoreDNS, authoritative for `MFA_DOMAIN` and forwarding everything else to
`MFA_DNS_UPSTREAM`, and `bigip/apm-build.sh` creates a forward-zone resolver pointing at
`${MFA_HOST_IP}:${MFA_DNS_PORT}`.

The zone is rendered from [`dns/Corefile.tmpl`](../../dns/Corefile.tmpl) and holds exactly
one record — `MFA_WEBTOP_FQDN` at `MFA_APM_VIP` — with `fallthrough` set, so any other name
falls through to the upstream forwarder rather than returning NXDOMAIN. Pointing a
workstation at this resolver as well is safe — non-demo names are forwarded — and it removes
the browser hosts-file step entirely.

## 2. Directory — both modes
| Variable | Default | Meaning |
|---|---|---|
...
The two modes are a configuration change, not a different design. In both, the entire
interaction with the directory is: bind to search, bind as the user to verify a password,
read `memberOf`. Nothing is ever written, which is what makes it safe to aim `external` mode
at a production AD.
...
([troubleshooting.md](../operations/troubleshooting.md#every-login-is-denied-and-the-log-blames-the-bind-account)).
...
### Connection details
These are **not** external-only, despite where an earlier `.env.example` put them. APM
queries the directory over `MFA_LDAP_PORT` and the BIG-IP's system authentication uses
`MFA_LDAPS_PORT`, both of them against the bundled directory as well. The DN defaults are
derived from `BASE_DN`, and the bundled seed is templated to match them.

| Variable | Default | Meaning |
|---|---|---|
| `MFA_LDAP_PORT` | `389` | Plain LDAP. Used by the APM AAA bind |
| `MFA_LDAPS_PORT` | `636` | LDAPS. Used by the BIG-IP's own system authentication, which is TLS-only |
...
| `MFA_USER_SEARCH_BASE` | `ou=people,${BASE_DN}` | Subtree identities are searched in — the only subtree in play |
| `MFA_LOGIN_ATTR` | `uid`, or `sAMAccountName` when `MFA_LDAP_SCHEMA=ad` | Attribute a username is matched against, in the APM AAA filter and in the BIG-IP's system auth |

`MFA_LOGIN_ATTR` is the one value derived from `MFA_LDAP_SCHEMA`, which is why setting the
schema is usually enough and setting the attribute by hand is rarely needed: the two
directory families disagree about it, and getting them out of step breaks every login rather
than one.

### Who gets which BIG-IP role
| Variable | Default | Meaning |
|---|---|---|
| `MFA_ADMIN_GROUP_DN` | `cn=bigip-admins,ou=groups,${BASE_DN}` | The group whose members get Administrator on **every** configured BIG-IP |
| `MFA_OPERATOR_GROUP_DN` | `cn=bigip-operators,ou=groups,${BASE_DN}` | The group granted the Operator role |
| `MFA_AUDITOR_GROUP_DN` | `cn=bigip-auditors,ou=groups,${BASE_DN}` | The group granted the Auditor role |
...
| `MFA_OPERATOR_ROLE_ATTRIBUTE` | `memberOf=${MFA_OPERATOR_GROUP_DN}` | The same, for Operator |
| `MFA_AUDITOR_ROLE_ATTRIBUTE` | `memberOf=${MFA_AUDITOR_GROUP_DN}` | The same, for Auditor |

These pairs are the entire authorization story, and they are evaluated by the target BIG-IP,
not by APM — see [ADR 0003](../adr/0003-authorization-on-remote-role.md).
[`bigip/system-auth.sh`](../../bigip/system-auth.sh) writes one `auth remote-role` rule per
group on each configured unit:

| Rule | Attribute | Role | `lineOrder` | Console |
|---|---|---|---|---|
| `bigip_mgt_mfa_admins` | `MFA_ADMIN_ROLE_ATTRIBUTE` | `administrator` | 1 | `tmsh` |
| `bigip_mgt_mfa_operators` | `MFA_OPERATOR_ROLE_ATTRIBUTE` | `operator` | 2 | disabled |
| `bigip_mgt_mfa_auditors` | `MFA_AUDITOR_ROLE_ATTRIBUTE` | `auditor` | 3 | disabled |

Rules are evaluated in `lineOrder`, most privileged first, so a user in two groups receives
the higher role rather than whichever rule happened to be evaluated last. Everyone who
authenticates and matches no rule falls through to `remote-user defaultRole: guest` with
remote console access disabled. Nothing about the read-only user is configured anywhere —
read-only is the default, and the roles above are the exceptions.
...
The role attributes are deliberately expressed as real group membership rather than an
...
them only if your directory expresses group membership under a different attribute —
Active Directory nested groups, for instance, may need a matching rule rather than a literal
`memberOf`. The value is written verbatim into the remote-role object, and
`scripts/validate.sh` asserts that what came back from
`/mgmt/tm/auth/remote-role/role-info/bigip_mgt_mfa_admins` equals this string.
...
## 3. Directory — bundled only
Ignored when `MFA_DIRECTORY_MODE=external`. Nothing here touches your own directory.

| Variable | Default | Meaning |
|---|---|---|
| `MFA_LDAP_ADMIN_PW` | _(required in bundled mode)_ | `cn=admin,${BASE_DN}` password for the bundled OpenLDAP. Used by the container and by the seeding `ldapadd` calls in `deploy.sh` |
...
`scripts/validate.sh` asserts both halves of the distinction — that alice carries `memberOf`
and that bob does not — because if either is wrong the role outcome on TMUI is wrong too.
...
#### `MFA_TEST_USER_PW`
Type: string. Default: none. Required: bundled mode, unless every `MFA_PW_*` above is set.

The fallback for any per-user password left blank. It exists so an older `.env` keeps
working; setting the per-user values is strongly preferred. In external mode nothing is
seeded and this key is unused, except that `scripts/validate.sh` still submits it as the
password for whichever principals it probes.

## 4. Directory — external only
Ignored when `MFA_DIRECTORY_MODE=bundled`, where each falls back to the derived value in the
default column. Validate the whole block with `scripts/preflight-directory.sh` **before**
touching a BIG-IP.
...
| `MFA_LDAP_SCHEMA` | `openldap` | `openldap` or `ad`. Selects the login attribute — see [Connection details](#connection-details) |

## 5. The BIG-IP
Skipped entirely by `./deploy.sh --stack`, which contacts no BIG-IP.
...
| `BIGIP_B_MGMT` | _(optional)_ | Unit B's management address. Receives system authentication and the remote-role rules only |
...
| `MFA_WEBTOP_FQDN` | `webtop.bigip-mgt-mfa.lab` | The name on the VIP certificate |
...
([`scripts/gen-certs.sh`](../../scripts/gen-certs.sh)), the single record in the CoreDNS zone
([`dns/Corefile.tmpl`](../../dns/Corefile.tmpl)), and the name `scripts/validate.sh` and
`scripts/test-mfa-matrix.sh` `--resolve` to `MFA_APM_VIP`. Keep it inside `MFA_DOMAIN` or the
demo zone will not answer for it. The certificate carries `IP:${MFA_APM_VIP}` as a second
SAN, so a browser that reaches the VIP by address still completes the redirect.
...
## 7. Certificates and rarely-changed overrides
The first three appear in `.env.example`, commented out with their defaults. The last two do
not appear there at all: `MFA_REGEN_CA` is an operator action rather than a setting, and
`MFA_FACADE_SNAT_ADDR` is derived. Each is written with a `${VAR:-default}` fallback, so any
of them can be overridden by exporting it or adding it to `.env`.

| Variable | Default | Meaning |
|---|---|---|
| `MFA_CA_CN` | `bigip-mgt-mfa Demo CA` | Subject CN of the generated CA. Quote it — the default contains spaces |
...
| `MFA_ORG` | `bigip-mgt-mfa` | Bundled OpenLDAP organisation name — see [§3](#3-directory--bundled-only) |
| `MFA_REGEN_CA` | `0` | Set to `1` for a single run to mint a **new** demo CA instead of reusing the existing one. Invalidates every browser trust import and both BIG-IP anchors, and needs `./deploy.sh --bigip` afterwards to reinstall them |
...
| Remote-role rules | `bigip_mgt_mfa_admins`, `bigip_mgt_mfa_operators`, `bigip_mgt_mfa_auditors` |
| TOTP seed data group | `bigip_mgt_mfa_totp_dg` |
