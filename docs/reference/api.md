...
The three interfaces warden-lite talks to, what it calls on each, and what the answers mean.

_Last validated: 2026-07._

## Overview
**warden-lite publishes no API of its own.** There is no service in this repo that listens
...
- **Site-specific values are placeholders here.** `<BIGIP_A_MGMT>`, `<BIGIP_B_MGMT>`,
  `<WL_KEYCLOAK_FQDN>`, `<WL_KEYCLOAK_PORT>`, `<WL_KEYCLOAK_REALM>`, `<WL_WEBTOP_FQDN>`,
  `<WL_APM_VIP>`, `<WL_SHADOW_A>` and `<WL_SHADOW_B>` all come from `.env` — see
...
certificate is the device's own self-signed one and warden-lite installs no trust for it.
...
| `/tm/auth/remote-role/role-info`, `/tm/auth/remote-role/role-info/warden_lite_admins` | GET, POST, PATCH, DELETE | `bigip/system-auth.sh`, `scripts/validate.sh`, `teardown.sh` | The one rule that grants `administrator`, matching `WL_ADMIN_ROLE_ATTRIBUTE`. The validator reads `.attribute` back and compares it to the configured string |
...
| `/tm/ltm/virtual`, `/tm/ltm/virtual/~Common~warden-lite-vs` | GET, POST, DELETE | `bigip/apm-build.sh`, `scripts/validate.sh` | The webtop virtual server on `<WL_APM_VIP>:443` and the two shadow façade virtual servers |
...
| `/tm/apm/resource/portal-access` | GET, POST | `bigip/apm-build.sh`, `scripts/validate.sh` | One Portal Access resource per unit, targeting the shadow façade rather than the real self IP. The validator asserts exactly two resources whose names start with `warden-lite-bigip` |
...
| `/tm/cm/device-group` | GET | `deploy.sh`, `teardown.sh` | Discovers the first `sync-failover` group when `WL_DEVICE_GROUP` is empty. No group means standalone, and the sync step is skipped rather than failed |
...
### Keycloak OIDC — `https://<WL_KEYCLOAK_FQDN>:<WL_KEYCLOAK_PORT>`
Everything the demo uses lives under `/realms/<WL_KEYCLOAK_REALM>/`. Two different clients
...
| `/admin/` | GET | an operator | The Keycloak administration console, as `<WL_KEYCLOAK_ADMIN>`. `deploy.sh` prints the URL when the stack half completes |

The issuer is the thing to watch. `KC_HOSTNAME` pins it to
`https://<WL_KEYCLOAK_FQDN>:<WL_KEYCLOAK_PORT>` so it cannot float with the request `Host`
...
### APM data plane — `https://<WL_WEBTOP_FQDN>/` (the VIP `<WL_APM_VIP>`)
| Endpoint | Purpose |
|---|---|
| `https://<WL_WEBTOP_FQDN>/` | The front door. An unauthenticated request establishes an APM session and is redirected to the logon page; an authenticated one is served the webtop |
| `https://<WL_WEBTOP_FQDN>/my.policy` | The access policy endpoint. GET serves the logon page; POST submits `username`, `password` and `vhost=standard`, which drives the LDAP Auth item. On success the response redirects to Keycloak's authorization endpoint |
| `https://<WL_WEBTOP_FQDN>/oauth/client/redirect` | The OIDC `redirect_uri`, configured identically on the APM OAuth agent and in the Keycloak client's `redirectUris`. Keycloak returns the browser here with the authorization code, and APM does the back-channel token exchange from this point |
...
| `https://<WL_SHADOW_A>/tmui/login.jsp`, `https://<WL_SHADOW_B>/tmui/login.jsp` | The `applicationUri` of the two Portal Access resources. These RFC 5737 addresses are not routable and are never browsed directly: a plain LTM virtual server listens on each, and a `node` iRule makes the real hop to `BIGIP_A_TMUI` or `BIGIP_B_TMUI`. They exist because APM Portal Access refuses reserved targets such as self IPs |

The webtop tile is where form SSO takes over: the portal engine fetches TMUI's login form and
`warden-lite-tmui-sso` posts the username and password already held in the session. The user
...
| a redirect back to `<WL_WEBTOP_FQDN>` | after the OTP form | The second factor succeeded and the authorization code is on its way to APM |

### APM data plane
| Result | Operational meaning |
|---|---|
| `200` or `302` at `https://<WL_WEBTOP_FQDN>/` | The VIP is listening and the access profile is attached. `scripts/validate.sh` accepts either |
...
| a redirect that does not point at `<WL_KEYCLOAK_FQDN>` | The OAuth agent did not run, which usually means the policy graph is not the one you think it is |
...
|---|---|---|
| `application/json` | every iControl REST write, set explicitly on every `POST` and `PATCH` | The management API is JSON-only for the paths warden-lite uses. Omitting the header gets the body rejected, so every helper in both build scripts sets it |
