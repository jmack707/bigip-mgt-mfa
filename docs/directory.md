...
warden-lite ships a directory so the demo works with nothing but a Docker host, and it points
...
## What warden-lite does to your directory
...
```bash
WL_DIRECTORY_MODE=external
WL_LDAP_HOST=dc01.example.com
WL_LDAP_SCHEMA=ad                 # or openldap for FreeIPA / OpenLDAP / 389
WL_LDAP_CA_FILE=/path/to/dc-ca.pem
WL_BIND_DN=CN=bigip-bind,OU=Service,DC=example,DC=com
WL_BIND_PW=<bind account password>
WL_USER_SEARCH_BASE=OU=Users,DC=example,DC=com
WL_ADMIN_GROUP_DN=CN=bigip-admins,OU=Groups,DC=example,DC=com
```
...
Any of them can still be overridden explicitly — `WL_LOGIN_ATTR`, `WL_KC_LDAP_VENDOR`,
`WL_KC_LDAP_UUID_ATTR`, `WL_KC_LDAP_USER_CLASSES` — for a directory that does not match either
profile.

## LDAPS and the CA
The BIG-IP's `auth ldap system-auth` connects over LDAPS and validates the certificate. In
bundled mode warden-lite issues that certificate itself. In external mode, export the CA that
signed your directory's LDAPS certificate to a PEM file and point `WL_LDAP_CA_FILE` at it —
...
- Users log in as `sAMAccountName`, but their DN uses `CN=<display name>`. warden-lite only
  ever searches and binds, so it does not need to construct DNs — but keep it in mind when
  setting `WL_USER_SEARCH_BASE`.
- `memberOf` in Active Directory reflects direct membership. If your admin group is nested,
  ...
  members of, or set `WL_ADMIN_ROLE_ATTRIBUTE` to an attribute you do populate directly.
...
`${WL_KEYCLOAK_REALM}`, the same credentials will work for APM and for the BIG-IPs.

```bash
ldapwhoami -x -H "ldap://${WL_LDAP_HOST}:${WL_LDAP_PORT}" -D "${WL_BIND_DN}" -w "${WL_BIND_PW}"
ldapsearch -x -LLL -H "ldap://${WL_LDAP_HOST}:${WL_LDAP_PORT}" -D "${WL_BIND_DN}" -w "${WL_BIND_PW}" \
  -b "${WL_USER_SEARCH_BASE}" "(${WL_LOGIN_ATTR}=alice.admin)" memberOf
```
